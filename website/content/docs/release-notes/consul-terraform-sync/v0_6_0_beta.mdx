...
## CTS Service Registration With Consul
CTS by default will automatically register itself as a service in Consul with a health check. The default health check created is an HTTP check to the [health API endpoint](#health-endpoint). If CTS is stopped, it will deregister itself with Consul.

If CTS is unsuccessful at registering itself with Consul, then CTS will log the error and continue without exiting.

~> If ACLs are enabled in Consul, then the [Consul token](/docs/nia/configuration#token) must have the policy `service:write` for service registration to succeed.

### Configuration
To support registration, CTS now has an `id` configuration that is a global configuration and a `service_registration` configuration that is part of the `consul` configuration. All configurations are optional.
...
The options for the global configuration `id` are:

| Name            | Required | Type    | Description                     |Default                  |
| --------------- | -------- | ------- | ------------------------------- | ----------------------- |
| `id ` | Optional | string | The ID of the CTS instance. Will be used as the service ID for CTS if service registration is enabled. | Generated ID with the format `cts-<uuid>` |


The configuration options for `consul.service_registration` are:

| Name            | Required | Type    | Description                     |Default                  |
| --------------- | -------- | ------- | ------------------------------- | ----------------------- |
| `enabled ` | Optional | boolean | Whether CTS should automatically register itself as a service with Consul. | `true` |
...
| `default_check.enabled` | Optional | boolean | Whether CTS should create the default health check. | `true` |
